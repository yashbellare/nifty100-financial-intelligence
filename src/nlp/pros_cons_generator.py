    ...
    return len(usable) >= count and any(
        all(
            ...
            for left, right in zip(
                usable[index : index + count - 1], usable[index + 1 : index + count]
            )
        )
        for index in range(len(usable) - count + 1)
    )
...
def _emit(
    company_id: str, signal_type: str, rule_id: str, text: str, strength: float = 1.0
) -> dict[str, object]:
    return {
...
def evaluate_company(company_id: str, rows: pd.DataFrame) -> list[dict[str, object]]:
    ...
    latest_ebitda = (
        None if operating_profit is None else operating_profit + (depreciation or 0)
    )
    ...
    if _consecutive(roe, lambda value: value > 20, 3):
        signals.append(
            _emit(
                ...
                "P01",
                PRO_TEXT["P01"],
                latest_roe / 20 if latest_roe else 1,
            )
        )
    ...
    if revenue_cagr is not None and revenue_cagr > 15:
        signals.append(
            _emit(company_id, "pro", "P04", PRO_TEXT["P04"], revenue_cagr / 15)
        )
    if latest_opm is not None and latest_opm > 25:
        signals.append(
            _emit(company_id, "pro", "P05", PRO_TEXT["P05"], latest_opm / 25)
        )
    ...
    if (
        len(assets) >= 2
        and assets[-1] is not None
        and assets[-2] is not None
        and assets[-1] > assets[-2]
        and borrowings[-1] is not None
        and borrowings[-2] is not None
        and borrowings[-1] < borrowings[-2]
    ):
    ...
    if (
        ...
        and latest_de > 2
        and str(latest.get("broad_sector", "")).lower()
        not in {"financials", "financial services"}
    ):
        signals.append(
            _emit(
                company_id,
                "con",
                "C01",
                CON_TEXT["C01"].format(de=latest_de),
                latest_de / 2,
            )
        )
    if _consecutive(fcf, lambda value: value < 0, 3):
        signals.append(_emit(company_id, "con", "C02", CON_TEXT["C02"]))
    if _consecutive(opm, lambda value: value is not None, 3) and any(
        opm[index] > opm[index + 1] > opm[index + 2]
        for index in range(len(opm) - 2)
        if all(value is not None for value in opm[index : index + 3])
    ):
    ...
    if latest_icr is not None and latest_icr < 1.5:
        signals.append(
            _emit(company_id, "con", "C06", CON_TEXT["C06"], (1.5 - latest_icr) + 1)
        )
    if latest_payout is not None and latest_payout > 100:
        signals.append(
            _emit(company_id, "con", "C07", CON_TEXT["C07"], latest_payout / 100)
        )
    if _improving(de, 3):
        signals.append(_emit(company_id, "con", "C08", CON_TEXT["C08"]))
    if _consecutive(eps, lambda value: value is not None, 3) and any(
        eps[index] > eps[index + 1] > eps[index + 2]
        for index in range(len(eps) - 2)
        if all(value is not None for value in eps[index : index + 3])
    ):
        signals.append(_emit(company_id, "con", "C09", CON_TEXT["C09"]))
    if latest_roce is not None and latest_roce < 10:
        signals.append(
            _emit(
                company_id, "con", "C10", CON_TEXT["C10"], (10 - latest_roce) / 10 + 1
            )
        )
    if (
        latest_net_debt is not None
        and latest_ebitda is not None
        and latest_ebitda > 0
        and latest_net_debt > 3 * latest_ebitda
    ):
        signals.append(
            _emit(
                company_id,
                ...
                "C11",
                CON_TEXT["C11"],
                latest_net_debt / (3 * latest_ebitda),
            )
        )
    if revenue_cagr is not None and revenue_cagr < 5:
        signals.append(
            _emit(company_id, "con", "C12", CON_TEXT["C12"], (5 - revenue_cagr) / 5 + 1)
        )
    ...
    if not pros:
        signals.append(
            _emit(
                ...
                "pro",
                "P00",
                "Financial data does not trigger a named positive signal; review the company fundamentals.",
                0,
            )
        )
    if not cons:
        signals.append(
            _emit(
                ...
                "con",
                "C00",
                "Financial data does not trigger a named risk signal; continue monitoring the company fundamentals.",
                0,
            )
        )
    return signals
...
def run(db_path: Path = DEFAULT_DB, output_path: Path = DEFAULT_OUTPUT) -> pd.DataFrame:
    """Generate and persist the pros and cons output."""
...
def main(argv: Iterable[str] | None = None) -> None:
    """Run the pros and cons command-line workflow."""
    ...
    print(
        f"Companies with both signal types: {int(((coverage.get('pro', 0) > 0) & (coverage.get('con', 0) > 0)).sum())}"
    )
