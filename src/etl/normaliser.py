...
def normalize_year(value):
    """Normalize a financial period through the loader implementation."""
...
def deduplicate(name, df):
    """Drop duplicate natural keys and return an audit log."""
    keys = {
    ...
    logs = []
    if len(dup):
        for vals, g in dup.groupby(keys, dropna=False):
            logs.append(
                (
                    name,
                    *([str(v) for v in (vals if isinstance(vals, tuple) else (vals,))]),
                    "keep_last",
                    len(g),
                )
            )
