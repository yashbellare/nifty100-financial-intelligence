...
RAW = BASE / "data/raw"
SUP = BASE / "data/supporting"
DB = BASE / os.getenv("DB_PATH", "nifty100.db")
OUT = BASE / os.getenv("OUTPUT_DIR", "output")


def normalize_ticker(value):
    """Normalize a ticker to uppercase text."""
    if pd.isna(value):
        return None
...
def normalize_year(value):
    """Normalize supported financial-period formats to YYYY-MM."""
    if pd.isna(value):
        return None
...
def read_excel(path, core_file=False):
    """Read a source workbook using its configured header row."""
    return pd.read_excel(path, header=1 if core_file else 0)


def clean_df(name, df):
    """Normalize identifiers, dates, and years in a source dataframe."""
    df = df.copy()
    df.columns = [str(c).strip() for c in df.columns]
    if "company_id" in df.columns:
        df["company_id"] = df["company_id"].map(normalize_ticker)
    if (
        name in {"profitandloss", "balancesheet", "cashflow", "financial_ratios"}
        and "year" in df.columns
    ):
        df["year"] = df["year"].map(normalize_year)
    if name == "documents" and "Year" in df.columns:
        df["Year"] = pd.to_numeric(df["Year"], errors="coerce").astype("Int64")
    if name == "stock_prices" and "date" in df.columns:
        df["date"] = pd.to_datetime(df["date"], errors="coerce").dt.strftime("%Y-%m-%d")
    return df


def load_all():
    """Load, validate, deduplicate, and persist all project datasets."""
    ...
    datasets = {}
    specs = [
        ("companies", RAW / "companies.xlsx", True),
        ("profitandloss", RAW / "profitandloss.xlsx", True),
        ("balancesheet", RAW / "balancesheet.xlsx", True),
        ("cashflow", RAW / "cashflow.xlsx", True),
        ("analysis", RAW / "analysis.xlsx", True),
        ("documents", RAW / "documents.xlsx", True),
        ("prosandcons", RAW / "prosandcons.xlsx", True),
        ("sectors", SUP / "sectors.xlsx", False),
        ("stock_prices", SUP / "stock_prices.xlsx", False),
        ("market_cap", SUP / "market_cap.xlsx", False),
        ("financial_ratios", SUP / "financial_ratios.xlsx", False),
        ("peer_groups", SUP / "peer_groups.xlsx", False),
    ]
    for n, p, c in specs:
        datasets[n] = clean_df(n, read_excel(p, c))
    ...
    datasets["companies"] = companies.drop_duplicates("id", keep="last")
    for n in [
        "profitandloss",
        "balancesheet",
        "cashflow",
        ...
        before = len(datasets[n])
        datasets[n], log = deduplicate(n, datasets[n])
        dedup_logs.extend(log)
        after = len(datasets[n])
    ...
    failures.extend(
        [
            x
            for x in raw_failures
            if x["rule_id"] in {"DQ-01", "DQ-02", "DQ-03", "DQ-07", "DQ-08"}
        ]
    )
    ...
    vf["status"] = vf.apply(
        lambda r: (
            "RESOLVED"
            if r.rule_id in {"DQ-01", "DQ-02", "DQ-03", "DQ-07", "DQ-08"}
            else "OPEN"
        ),
        axis=1,
    )
    ...
    exec_rows = []
    for rule in rules:
        if rule == "DQ-13" and os.getenv("DQ_URL_CHECK", "false").lower() != "true":
            status = "NOT_RUN_OFFLINE"
            findings = 0
        else:
            sub = vf[vf.rule_id == rule]
            findings = len(sub)
            status = (
                "PASS"
                if findings == 0
                else ("RESOLVED" if (sub.status == "RESOLVED").all() else "OPEN")
            )
    ...
    for n, df in datasets.items():
        ...
        if n in {"profitandloss", "balancesheet", "cashflow", "financial_ratios"}:
            mask = (
                df.year.notna()
                & df.company_id.notna()
                & df.company_id.str.len().between(2, 12)
            )
            rejected += int((~mask).sum())
            df = df[mask].copy()
        if n == "stock_prices":
        ...
        if n == "peer_groups":
            df["is_benchmark"] = df["is_benchmark"].fillna(False).astype(int)
        # pandas NaN -> SQL NULL
    ...
    (OUT / "deduplication_log.csv").write_text(
        "table,company_id,year,action,source_rows\n"
        + "".join(",".join(map(str, x)) + "\n" for x in dedup_logs),
        encoding="utf-8",
    )
...
if __name__ == "__main__":
    d, f, fk = load_all()
    print("Loaded", len(d), "tables; failures=", len(f), "FK=", len(fk))
