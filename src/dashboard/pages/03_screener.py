...
def load_screener_data():
    """Build the dashboard screener dataframe from shared data loaders."""
    ...
    latest_ratios = pd.DataFrame(ratio_rows).drop(
        columns=["year_number"], errors="ignore"
    )
    latest_market = pd.DataFrame(market_rows).drop(
        columns=["year_number"], errors="ignore"
    )
    ...
    exports = (
        pd.concat(export_rows, ignore_index=True) if export_rows else pd.DataFrame()
    )
    if not exports.empty:
        exports["year_number"] = exports.get(
            "year", pd.Series(0, index=exports.index)
        ).map(_year_number)
        exports = (
            exports.sort_values("year_number")
            .groupby("company_id", as_index=False)
            .last()
        )

    result = companies.merge(
        latest_ratios, on="company_id", how="left", suffixes=("", "_ratio")
    )
    result = result.merge(
        latest_market[
            [
    ...
    if not exports.empty:
        result = result.merge(
            exports, on="company_id", how="left", suffixes=("", "_export")
        )
    ...
    aliases = {
        ...
        "dividend_yield": [
            "dividend_yield_export",
            "dividend_yield",
            "dividend_yield_pct",
        ],
        "composite_score": [
            "composite_quality_score_export",
    ...
    result["composite_score"] = _numeric(
        result.get("composite_score", pd.Series(index=result.index))
    )
    if result["composite_score"].isna().all():
        ...
        if "debt_to_equity" in result:
            score_parts.append(
                _numeric(result["debt_to_equity"]).rank(pct=True, ascending=False)
            )
        result["composite_score"] = (
            pd.concat(score_parts, axis=1).mean(axis=1).mul(100)
            if score_parts
            else pd.NA
        )
    return result
...
st.caption(
    "Tune the thresholds or start with a preset. Companies with unavailable data do not pass that filter."
)
...
with st.sidebar:
    ...
    for index, preset_name in enumerate(PRESETS):
        if preset_columns[index % 2].button(
            preset_name, key=f"preset_{preset_name}", width="stretch"
        ):
...
results = data.loc[
    mask, [column for column in visible_columns if column in data.columns]
].copy()
