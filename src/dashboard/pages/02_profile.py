...
from src.dashboard.utils.db import get_bs, get_cf, get_companies, get_pl, get_pros_cons, get_ratios, get_sectors
...
search = st.text_input("Search by company name or ticker", placeholder="Type a company name or ticker")
search_text = search.strip().lower()
if search_text:
    matches = companies[companies["company_id"].astype(str).str.lower().str.contains(search_text, na=False) | companies["company_name"].astype(str).str.lower().str.contains(search_text, na=False)]
else:
    matches = companies
...
ticker = st.selectbox("Select company", options, format_func=lambda value: labels.get(value, value))
...
kpis[2].metric("Net profit margin", format_metric(number(latest_ratios.get("net_profit_margin_pct")), "%"))
kpis[3].metric("D/E", format_metric(number(latest_ratios.get("debt_to_equity"))))
kpis[4].metric("Revenue CAGR 5Y", format_metric(number(latest_ratios.get("revenue_cagr_5yr")), "%"))
kpis[5].metric("FCF", format_metric(number(latest_ratios.get("free_cash_flow_cr")), " cr"))
...
if pl.empty:
    ...
    bar.update_layout(barmode="group", height=400, margin=dict(l=10, r=10, t=30, b=10), yaxis_title="INR crore")
    st.plotly_chart(bar, width="stretch")

    trend = []
    for _, row in chart_data.iterrows():
        balance = bs[bs["year_number"] == row["Year"]].tail(1) if not bs.empty else pd.DataFrame()
        capital = 0 if balance.empty else sum(number(balance.iloc[0].get(column)) or 0 for column in ["equity_capital", "reserves", "borrowings"])
        source_pl = pl[pl["year_number"] == row["Year"]].tail(1)
        roce_value = None
        if not source_pl.empty and capital > 0:
            roce_value = (number(source_pl.iloc[0].get("operating_profit")) or 0) / capital * 100
        ratio = ratios[ratios["year_number"] == row["Year"]].tail(1) if not ratios.empty else pd.DataFrame()
        trend.append({"Year": row["Year"], "ROE": number(ratio.iloc[0].get("return_on_equity_pct")) if not ratio.empty else None, "ROCE": roce_value})
    trend_data = pd.DataFrame(trend)
    line = make_subplots(specs=[[{"secondary_y": True}]])
    line.add_trace(go.Scatter(x=trend_data["Year"], y=trend_data["ROE"], name="ROE", mode="lines+markers"), secondary_y=False)
    line.add_trace(go.Scatter(x=trend_data["Year"], y=trend_data["ROCE"], name="ROCE", mode="lines+markers"), secondary_y=True)
