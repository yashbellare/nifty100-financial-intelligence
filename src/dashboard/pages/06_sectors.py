...
def sector_data():
    """Build the sector analysis dataframe."""
        ...
        rows.append(
            {
                ...
                "ROE": pd.to_numeric(
                    ratios.get("return_on_equity_pct"), errors="coerce"
                ),
                "D/E": pd.to_numeric(ratios.get("debt_to_equity"), errors="coerce"),
                "Net profit margin": pd.to_numeric(
                    ratios.get("net_profit_margin_pct"), errors="coerce"
                ),
                "Market cap": pd.to_numeric(
                    market.get("market_cap_crore"), errors="coerce"
                ),
            }
        )
...
figure = px.scatter(
    ...
    hover_data=["company_id", "D/E"],
    size_max=48,
)
figure.update_layout(
    height=520, margin=dict(l=10, r=10, t=35, b=10), legend_title="Sub-sector"
)
if not plot_data.empty:
    st.plotly_chart(figure, width="stretch")

median_data = (
    selected[["ROE", "D/E", "Net profit margin"]]
    .median()
    .rename_axis("Metric")
    .reset_index(name="Median")
)
bar = px.bar(
    median_data, x="Metric", y="Median", color="Metric", title=f"{sector} median KPIs"
)
