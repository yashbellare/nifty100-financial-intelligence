...
metric_labels = st.multiselect(
    "Metrics", list(METRICS), default=list(METRICS)[:2], max_selections=3
)
...
selected_columns = [
    METRICS[label] for label in metric_labels if METRICS[label] in history.columns
]
if not selected_columns:
    st.info("No selected metrics are available for this company.")
    st.stop()

chart_data = history.groupby("Year", as_index=False)[selected_columns].mean(
    numeric_only=True
)
chart_data[selected_columns] = chart_data[selected_columns].apply(
