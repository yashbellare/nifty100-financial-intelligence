...
def load_allocation():
    """Load and normalize capital allocation output data."""
...
data = data.merge(companies, on="company_id", how="left").merge(
    sectors, on="company_id", how="left"
)
data["Company"] = data["company_name"].fillna(data["company_id"])
counts = data.groupby("pattern_label", as_index=False).agg(
...
selection = st.plotly_chart(
    figure, width="stretch", on_select="rerun", key="capital_treemap"
)
...
companies_in_pattern = data[data["pattern_label"] == selected_pattern][
    ["Company", "company_id", "broad_sector"]
].sort_values("Company")
