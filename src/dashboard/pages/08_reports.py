...
def check_report(url):
    """Check whether an annual-report URL has a valid HTTP scheme."""
    ...
    try:
        ...
        if response.status_code == 405:
            response = requests.get(
                str(url), stream=True, allow_redirects=True, timeout=3
            )
...
for _, report in documents.iterrows():
    ...
    rows.append(
        {
            "Year": int(report["year"]) if pd.notna(report["year"]) else "N/A",
            "url": url,
            "available": available,
            "status": status,
        }
...
for report in rows:
    ...
    if available:
        st.markdown(
            f"**{year}**  |  [Open BSE annual report]({url})  |  :green[Available]"
        )
    else:
