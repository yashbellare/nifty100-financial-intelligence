...
from src.dashboard.utils.db import get_companies, get_peer_group_names, get_peers, get_ratios, get_sectors
...
for ticker in peer_ids:
	...
	company = company_lookup.loc[ticker] if ticker in company_lookup.index else pd.Series(dtype="object")
	row = {"company_id": ticker, "company_name": company.get("company_name", ticker), "is_benchmark": False}
	...
	row["is_benchmark"] = bool(not benchmark.empty and benchmark.iloc[0].get("is_benchmark", False))
	rows.append(row)
...
selected = st.selectbox("Benchmark company", labels, index=labels.index(benchmark_ids[0]) if benchmark_ids else 0, format_func=lambda ticker: f"{company_lookup.loc[ticker, 'company_name']} ({ticker})" if ticker in company_lookup.index else ticker)

metric_columns = {
	"ROE (%)": "roe", "ROCE (%)": "roce", "NPM (%)": "net_profit_margin_pct", "D/E": "debt_to_equity",
