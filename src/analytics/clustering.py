...
def load_clustering_data(db_path: Path = DEFAULT_DB_PATH) -> pd.DataFrame:
    ...
    if missing_columns:
        raise ValueError(
            f"financial_ratios is missing columns: {sorted(missing_columns)}"
        )
    if "fcf_cagr_5yr" not in ratios.columns:
        ...
        fcf_cagrs = {}
        for company_id, rows in ratios.dropna(subset=["year_number"]).groupby(
            "company_id"
        ):
    ...
    ratios["year_sort"] = pd.to_numeric(
        ratios["year"]
        .astype(str)
        .str.extract(r"(\d{4})(?:-(\d{1,2}))?", expand=False)[0],
        errors="coerce",
    )
...
def _cluster_names(centers: pd.DataFrame) -> dict[int, str]:
    ...
    for name, score, descending in roles:
        ordered = profile.loc[list(remaining), score].sort_values(
...
def cluster_profile_summary(labels: pd.DataFrame, data: pd.DataFrame) -> pd.DataFrame:
...
def _write_elbow_plot(
    ...
    axis.set(
        title="KMeans Elbow Plot", xlabel="Number of clusters (k)", ylabel="Inertia"
    )
    axis.legend()
    fig.tight_layout()
    fig.savefig(path, dpi=150)
...
def main() -> None:
    ...
    print(
        f"Generated {len(labels)} cluster labels across {labels['cluster_id'].nunique()} clusters"
    )
