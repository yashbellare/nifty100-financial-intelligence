...
def net_profit_margin(net_profit, sales):
    ...
    if _none_if_zero(sales):
        return None
    return net_profit / sales * 100 if net_profit is not None else None


def operating_profit_margin(operating_profit, sales):
    """Calculate operating profit as a percentage of sales."""
    if _none_if_zero(sales):
        return None
...
def opm_crosscheck(computed, source, tolerance_pp=1.0):
    """Compare computed and source OPM values within a percentage-point tolerance."""
...
def return_on_capital_employed(ebit, equity_capital, reserves, borrowings):
    """Calculate ROCE using equity, reserves, and borrowings."""
...
def return_on_assets(net_profit, total_assets):
    ...
    if _none_if_zero(total_assets) or total_assets < 0 or net_profit is None:
        return None
    return net_profit / total_assets * 100


def debt_to_equity(borrowings, equity_capital, reserves):
    """Calculate debt to equity and return zero for debt-free companies."""
...
def high_leverage_flag(de, broad_sector):
    """Flag non-financial companies with debt to equity above five."""
    return bool(
        de is not None and de > 5 and str(broad_sector).strip().lower() != "financials"
    )


def interest_coverage(operating_profit, other_income, interest):
    """Calculate interest coverage from operating profit and other income."""
    if _none_if_zero(interest):
        return None
    return ((operating_profit or 0) + (other_income or 0)) / interest


def interest_coverage_label(icr):
    """Label a missing interest coverage value as debt free."""
    return "Debt Free" if icr is None else None


def interest_warning(icr, threshold=1.5):
    """Flag interest coverage below the warning threshold."""
    return bool(icr is not None and icr < threshold)


def net_debt(borrowings, investments):
    """Calculate net debt after investments."""
    return (borrowings or 0) - (investments or 0)


def asset_turnover(sales, total_assets):
    """Calculate sales generated per unit of total assets."""
    if _none_if_zero(total_assets):
        return None
    return sales / total_assets if sales is not None else None


def free_cash_flow(operating_activity, investing_activity):
    """Calculate free cash flow from operating and investing cash flow."""
...
def capex_intensity(investing_activity, sales):
    """Calculate capital expenditure as a percentage of sales."""
    if _none_if_zero(sales):
        return None
...
def capex_intensity_label(pct):
    """Classify capital intensity into asset-light, moderate, or intensive."""
    if pct is None:
        return None
    if pct < 3:
        return "Asset Light"
...
def capital_allocation_pattern(cfo, cfi, cff, cfo_pat_ratio=None):
    """Classify a company from the signs of its three cash-flow streams."""
    s = lambda x: (
        "+" if x is not None and x > 0 else "-" if x is not None and x < 0 else "0"
    )
...
def book_value_per_share(equity_capital, reserves, face_value=10):
    """Calculate book value per share from equity capital and reserves."""
