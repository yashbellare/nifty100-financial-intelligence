...
def load_capital_allocation(
    path: Path = PROJECT_ROOT / "output" / "capital_allocation.csv",
) -> pd.DataFrame:
    ...
    allocation["pattern_label"] = (
        allocation["pattern_label"].astype("string").str.strip()
    )
...
def validate_capital_allocation(
    allocation: pd.DataFrame, company_ids: pd.Series | list[str]
) -> pd.DataFrame:
    """Return one validation row per expected company and source integrity check."""
    expected = (
        pd.Series(company_ids, dtype="string")
        .astype(str)
        .str.strip()
        .str.upper()
        .drop_duplicates()
    ...
    for company_id in expected:
        ...
        duplicate_keys = int(
            company_rows.duplicated(["company_id", "year_number"]).sum()
        )
        rows.append(
            {
                "company_id": company_id,
                "status": (
                    "PASS" if company_id in present and duplicate_keys == 0 else "FAIL"
                ),
                ...
                "issue": (
                    ...
                    else ("duplicate company-year key" if duplicate_keys else "")
                ),
            }
        )
        ...
        rows.append(
            {
                "company_id": company_id,
                "status": "FAIL",
                "allocation_years": int((allocation["company_id"] == company_id).sum()),
                "duplicate_company_years": 0,
                "issue": "not present in companies table",
            }
        )
    return pd.DataFrame(
        rows,
...
def latest_allocation_distribution(allocation: pd.DataFrame) -> pd.DataFrame:
    ...
    latest = (
        allocation.sort_values(["company_id", "year_number"])
    ...
    return pd.DataFrame(
        {
            "pattern_label": ALLOCATION_PATTERNS,
            "company_count": [
                int(counts.get(pattern, 0)) for pattern in ALLOCATION_PATTERNS
            ],
        }
    )
...
def allocation_pattern_changes(allocation: pd.DataFrame) -> pd.DataFrame:
    ...
    changed = ordered[
        ordered["prior_pattern"].notna()
        & ordered["pattern_label"].ne(ordered["prior_pattern"])
    ].copy()
    return changed.rename(
        columns={
            "prior_year": "from_year",
            "year": "to_year",
            "prior_pattern": "from_pattern",
...
def compute_cashflow_intelligence(
    ...
    if sectors is not None and not sectors.empty:
        ...
        sector_column = (
            "broad_sector" if "broad_sector" in sectors else sectors.columns[-1]
        )
        sector_map = dict(
            zip(sectors[sector_id].astype(str).str.upper(), sectors[sector_column])
        )
    ...
    for company_id in company_ids:
            ...
            rows.append(
                {
                    ...
                    **{column: None for column in REQUIRED_COLUMNS[2:]},
                }
            )
            continue

        joined = cf_history[
            ["company_id", "year_number", "operating_activity", "investing_activity"]
        ].copy()
        joined = joined.merge(
            pl_history[["year_number", "sales", "net_profit"]],
            on="year_number",
            how="left",
        )
        joined["fcf"] = [
            free_cash_flow(_number(cfo), _number(cfi))
            for cfo, cfi in zip(joined.operating_activity, joined.investing_activity)
        ]
        quality_history = joined.merge(
            pl_history[["year_number", "net_profit"]].rename(
                columns={"net_profit": "pat"}
            ),
            on="year_number",
            how="left",
            suffixes=("", "_duplicate"),
        ...
        latest_pat = (
            _number(latest_pl.iloc[-1].net_profit) if not latest_pl.empty else None
        )
        ...
        prior_borrowings = (
            _number(prior_bs.iloc[-1].borrowings) if not prior_bs.empty else None
        )
        latest_borrowings = (
            _number(latest_bs.iloc[-1].borrowings) if not latest_bs.empty else None
        )
        distress = bool(
        ...
        deleveraging = bool(
            ...
            and prior_borrowings is not None
            and latest_borrowings < prior_borrowings
        )
        cfo_pat_ratio = (
            latest_cfo / latest_pat
            if latest_cfo is not None and latest_pat not in (None, 0)
            else None
        )
        allocation = capital_allocation_pattern(
            latest_cfo, latest_cfi, latest_cff, cfo_pat_ratio
        )
        conversion = (
            latest_fcf / latest_cfo * 100 if latest_cfo not in (None, 0) else None
        )
        result = {
            "company_id": company_id,
            "sector": sector_map.get(company_id),
    ...
    return pd.DataFrame(rows, columns=REQUIRED_COLUMNS), pd.DataFrame(
        alerts,
        columns=["company_id", "sector", "year", "cfo", "cff", "latest_net_profit"],
    )


def generate_cashflow_intelligence(
    db_path: Path = PROJECT_ROOT / "nifty100.db",
    ...
    result["capital_allocation_label"] = (
        ...
        .fillna(result["capital_allocation_label"])
    )
    latest_allocation_distribution(allocation).to_csv(
        output_dir / "capital_allocation_distribution.csv", index=False
    )
    allocation_pattern_changes(allocation).to_csv(
        output_dir / "pattern_changes.csv", index=False
    )
...
__all__ = [
    ...
    "latest_allocation_distribution",
    "allocation_pattern_changes",
    "cfo_quality_score",
    "cfo_quality_label",
    "capex_intensity",
    "capex_intensity_label",
