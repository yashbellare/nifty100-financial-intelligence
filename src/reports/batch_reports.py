...
from .portfolio import (
    DEFAULT_OUTPUT as DEFAULT_PORTFOLIO_OUTPUT,
...
def _company_coverage(db_path: Path) -> pd.DataFrame:
    with sqlite3.connect(db_path) as connection:
        ...
        history = pd.read_sql_query(
            "SELECT company_id, year FROM profitandloss", connection
        )
    history["year_number"] = history["year"].map(_year)
    coverage = (
        history.dropna(subset=["year_number"])
        .groupby("company_id")["year_number"]
        .nunique()
    )
    companies["years_available"] = (
        companies["company_id"].map(coverage).fillna(0).astype(int)
    )
    return companies
...
def generate_batch_reports(
    ...
    tearsheet_dir: Path = DEFAULT_OUTPUT_DIR,
    sector_dir: Path = DEFAULT_OUTPUT,
    skipped_path: Path = DEFAULT_SKIPPED,
) -> dict[str, object]:
    ...
    skipped[["company_id", "company_name", "years_available", "reason"]].to_csv(
    ...
    for ticker in coverage.loc[coverage["years_available"] >= 3, "company_id"]:
        try:
            generated.append(
                generate_tearsheet(
                    ticker, tearsheet_dir / f"{ticker}_tearsheet.pdf", db_path
                )
            )
    ...
    if errors:
        pd.DataFrame(errors).to_csv(
            tearsheet_dir.parent / "tearsheet_errors.csv", index=False
        )
    return {
        "generated": generated,
        "skipped": skipped,
        "sectors": sector_paths,
...
def main(argv: Iterable[str] | None = None) -> None:
    """Run the batch report command-line workflow."""
    ...
    result = generate_batch_reports(
        args.db, args.tearsheet_dir, args.sector_dir, args.skipped
    )
