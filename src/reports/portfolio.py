...
def load_portfolio_data(db_path: Path = DEFAULT_DB) -> pd.DataFrame:
    ...
    with sqlite3.connect(db_path) as connection:
        ...
        pl = pd.read_sql_query(
            "SELECT company_id, year, net_profit FROM profitandloss", connection
        )
    ...
    ratios = ratios.dropna(subset=["year_number"]).sort_values(
    ...
    latest = (
        latest.merge(companies, on="company_id", how="left")
        .merge(sectors, on="company_id", how="left")
        .merge(prior, on="company_id", how="left")
    )
    latest_pl = pl.groupby("company_id", as_index=False).tail(1)[
        ["company_id", "net_profit"]
    ]
    latest_cf = cf.groupby("company_id", as_index=False).tail(1)
    latest = latest.merge(latest_pl, on="company_id", how="left").merge(
        latest_cf[["company_id", "operating_activity", "investing_activity"]],
        on="company_id",
        how="left",
    )
    latest["free_cash_flow_cr"] = pd.to_numeric(
        latest["operating_activity"], errors="coerce"
    ) + pd.to_numeric(latest["investing_activity"], errors="coerce")
...
def _metric_rows(
    row: pd.Series, styles: dict[str, ParagraphStyle]
) -> list[list[object]]:
    metrics = [
    ...
    rows = [
        [
            Paragraph("Metric", styles["heading"]),
            Paragraph("Latest", styles["heading"]),
            Paragraph("Trend", styles["heading"]),
        ]
    ]
    for label, column, suffix, decimals in metrics:
        ...
        rows.append(
            [
                ...
                Paragraph(_fmt(row.get(column), suffix, decimals), styles["cell"]),
...
def generate_portfolio_summary(
    db_path: Path = DEFAULT_DB, output_path: Path = DEFAULT_OUTPUT
) -> Path:
    ...
    for index, row in load_portfolio_data(db_path).iterrows():
        header = Table(
            [
                [
                    Paragraph(
                        str(row.get("company_name") or row["company_id"]),
                        styles["title"],
                    ),
                    Paragraph(str(row["company_id"]), styles["title"]),
                ]
            ],
        ...
        story.extend(
            [
                ...
                Spacer(1, 6 * mm),
                Paragraph(
                    f"{row.get('broad_sector') or 'Unclassified'}  |  Latest available year: {int(row['year_number'])}",
                    styles["subhead"],
                ),
                Spacer(1, 8 * mm),
            ]
        ...
        story.extend(
            [
                ...
                Spacer(1, 10 * mm),
                Paragraph(
                    "Trend arrows compare the latest value with the preceding available year; changes within 2% are shown as flat.",
                    styles["small"],
                ),
            ]
        )
...
def main(argv: Iterable[str] | None = None) -> None:
    """Run the portfolio report command-line workflow."""
