...
def load_company_data(ticker: str, db_path: Path = DEFAULT_DB, output_dir: Path = PROJECT_ROOT / "output") -> dict[str, object]:
    """Load all source data needed for one tearsheet."""
    ticker = str(ticker).strip().upper()
    with sqlite3.connect(db_path) as connection:
        company = pd.read_sql_query("SELECT * FROM companies WHERE id = ?", connection, params=[ticker])
        if company.empty:
            raise ValueError(f"Unknown company ticker: {ticker}")
        tables = {}
        for table in ("profitandloss", "balancesheet", "cashflow", "financial_ratios"):
            tables[table] = pd.read_sql_query(f"SELECT * FROM {table} WHERE company_id = ?", connection, params=[ticker])
        sector = pd.read_sql_query("SELECT * FROM sectors WHERE company_id = ?", connection, params=[ticker])
    ...
    if allocation_path.exists():
        ...
        allocation = allocation[allocation["company_id"].astype(str).str.upper() == ticker]
...
def _paragraph(pdf: canvas.Canvas, text: str, x: float, y_top: float, width: float, style: ParagraphStyle, max_height: float | None = None) -> float:
...
def _section(pdf: canvas.Canvas, title: str, x: float, y: float, width: float, style: ParagraphStyle) -> None:
    pdf.setFillColor(NAVY)
    pdf.rect(x, y - 3, width, 1.5, fill=1, stroke=0)
    _paragraph(pdf, title, x, y - 7, width, style)


def _draw_header(pdf: canvas.Canvas, company: pd.Series, sector: pd.Series, ticker: str, page: int) -> None:
    pdf.setFillColor(NAVY)
...
def _draw_kpi_tiles(pdf: canvas.Canvas, ratios: pd.DataFrame, pl: pd.DataFrame, bs: pd.DataFrame, x: float, y: float, width: float, styles: dict[str, ParagraphStyle]) -> None:
...
def _chart_frame(pdf: canvas.Canvas, title: str, x: float, y: float, width: float, height: float) -> tuple[float, float, float, float]:
    pdf.setFillColor(colors.white)
    pdf.setStrokeColor(GRID)
...
def _draw_bar_chart(pdf: canvas.Canvas, frame: pd.DataFrame, x: float, y: float, width: float, height: float) -> None:
    left, bottom, plot_width, plot_height = _chart_frame(pdf, "Revenue and net profit (INR cr)", x, y, width, height)
    values = frame[["sales", "net_profit"]].apply(pd.to_numeric, errors="coerce").fillna(0)
    ...
    for row_index, (_, row) in enumerate(frame.iterrows()):
        ...
        for offset, (column, color) in enumerate((("sales", BLUE), ("net_profit", GREEN))):
...
def _draw_line_chart(pdf: canvas.Canvas, frame: pd.DataFrame, x: float, y: float, width: float, height: float) -> None:
    left, bottom, plot_width, plot_height = _chart_frame(pdf, "ROE and ROCE trend (%)", x, y, width, height)
    ...
    for column, color in (("roe", RED), ("roce", BLUE)):
        ...
        if len(points) > 1:
            ...
            pdf.lines([(points[index][0], points[index][1], points[index + 1][0], points[index + 1][1]) for index in range(len(points) - 1)])
        pdf.setFillColor(color)
        for px, py in points:
...
def _draw_stacked_balance(pdf: canvas.Canvas, frame: pd.DataFrame, x: float, y: float, width: float, height: float) -> None:
    left, bottom, plot_width, plot_height = _chart_frame(pdf, "Balance sheet composition (INR cr)", x, y, width, height)
    ...
    maximum = max(float((values[["equity", "borrowings", "other"]].sum(axis=1)).max()), 1.0)
...
def _draw_waterfall(pdf: canvas.Canvas, latest: pd.Series, x: float, y: float, width: float, height: float) -> None:
    left, bottom, plot_width, plot_height = _chart_frame(pdf, "Latest-year cash flow (INR cr)", x, y, width, height)
    entries = [("CFO", _number(latest.get("operating_activity"))), ("CFI", _number(latest.get("investing_activity"))), ("CFF", _number(latest.get("financing_activity"))), ("Net", _number(latest.get("net_cash_flow")))]
...
def _draw_bullets(pdf: canvas.Canvas, items: list[str], x: float, y_top: float, width: float, style: ParagraphStyle, color: colors.Color) -> None:
    pdf.setFillColor(color)
    y = y_top
    for item in items[:6]:
...
def _render(data: dict[str, object], output_path: Path) -> Path:
    ...
    _draw_kpi_tiles(pdf, data["ratios"], data["pl"], data["bs"], margin, PAGE_HEIGHT - 42 * mm, content_width, styles)

    pl = data["pl"].copy()
    pl["year_number"] = pl["year"].map(_year)
    pl = pl.dropna(subset=["year_number"]).sort_values("year_number").tail(10)
    if not pl.empty:
        _draw_bar_chart(pdf, pl, margin, 103 * mm, (content_width - 5 * mm) / 2, 65 * mm)
    ratios = data["ratios"].copy()
    ratios["year_number"] = ratios["year"].map(_year)
    bs = data["bs"].copy()
    bs["year_number"] = bs["year"].map(_year)
    trend = ratios[["year_number", "return_on_equity_pct", "return_on_capital_employed_pct"]].rename(columns={"return_on_equity_pct": "roe", "return_on_capital_employed_pct": "roce"}).dropna(subset=["year_number"]).sort_values("year_number").tail(10)
    if not trend.empty:
        _draw_line_chart(pdf, trend, margin + (content_width + 5 * mm) / 2, 103 * mm, (content_width - 5 * mm) / 2, 65 * mm)
    _paragraph(pdf, f"<b>Coverage:</b> {len(pl)} annual observations shown; latest financial year: {_safe_text(_latest(data['pl']).get('year'))}", margin, 97 * mm, content_width, styles["small"])
    pdf.showPage()

    _draw_header(pdf, company, sector, ticker, 2)
    bs_chart = bs[["year_number", "equity_capital", "reserves", "borrowings", "other_liabilities"]].dropna(subset=["year_number"]).sort_values("year_number").tail(10).fillna(0)
    if not bs_chart.empty:
        _draw_stacked_balance(pdf, bs_chart, margin, 181 * mm, (content_width - 5 * mm) / 2, 55 * mm)
    latest_cf = _latest(data["cf"])
    if not latest_cf.empty:
        _draw_waterfall(pdf, latest_cf, margin + (content_width + 5 * mm) / 2, 181 * mm, (content_width - 5 * mm) / 2, 55 * mm)

    _section(pdf, "Pros", margin, 174 * mm, (content_width - 5 * mm) / 2, styles["section"])
    _section(pdf, "Cons", margin + (content_width + 5 * mm) / 2, 174 * mm, (content_width - 5 * mm) / 2, styles["section"])
    signals = data["pros_cons"]
    pros = signals[signals["type"].astype(str).str.lower() == "pro"]["text"].tolist() if not signals.empty else []
    cons = signals[signals["type"].astype(str).str.lower() == "con"]["text"].tolist() if not signals.empty else []
    column_width = (content_width - 5 * mm) / 2
    _draw_bullets(pdf, pros, margin, 168 * mm, column_width, styles["bullet"], GREEN)
    _draw_bullets(pdf, cons, margin + (content_width + 5 * mm) / 2, 168 * mm, column_width, styles["bullet"], RED)

    allocation = data["allocation"]
    latest_pattern = _safe_text(_latest(allocation).get("pattern_label"), "Not available") if not allocation.empty else "Not available"
...
def generate_tearsheet(ticker: str, output_path: Path | None = None, db_path: Path = DEFAULT_DB, output_dir: Path = PROJECT_ROOT / "output") -> Path:
...
def main(argv: Iterable[str] | None = None) -> None:
    ...
    tickers = [args.ticker] if args.ticker else ["TCS", "HDFCBANK", "RELIANCE", "SUNPHARMA", "TATASTEEL"]
    for ticker in tickers:
        path = generate_tearsheet(ticker, args.output_dir / f"{ticker}_tearsheet.pdf", args.db)
        print(path)
