...
@router.get("")
def list_sectors(connection: sqlite3.Connection = Depends(get_db)) -> list[dict[str, object]]:
...
def sector_companies(sector: str, connection: sqlite3.Connection = Depends(get_db)) -> list[dict[str, object]]:
