...
def peer_group(group_name: str, connection: sqlite3.Connection = Depends(get_db)) -> list[dict[str, object]]:
	...
	rows = connection.execute(
		"""SELECT p.company_id, c.company_name, p.metric, p.value, p.percentile_rank, p.year
		   ...
		   WHERE p.peer_group_name = ? ORDER BY c.company_name, p.metric""", [group_name]
	).fetchall()
	if not rows:
