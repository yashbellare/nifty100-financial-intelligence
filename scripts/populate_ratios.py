...
def compute():
    ...
    pl = pl.set_index(["company_id", "year"])
    bs = bs.set_index(["company_id", "year"])
    cf = cf.set_index(["company_id", "year"])
    log_rows = []
    rows = []
    for _, k in keys.iterrows():
        ...
        def g(df, key):
            return fnum(df[key]) if key in df.index else None

        sales, op, npf, oi, interest, eps, div = (
            g(p, "sales"),
            g(p, "operating_profit"),
            g(p, "net_profit"),
            g(p, "other_income"),
            g(p, "interest"),
            g(p, "eps"),
        ...
        cfo, cfi, cff = (
            g(c, "operating_activity"),
            g(c, "investing_activity"),
            g(c, "financing_activity"),
        )
        npm = net_profit_margin(npf, sales)
        opm = operating_profit_margin(op, sales)
        ...
        if cross:
            log_rows.append(
                (
                    ...
                    "OPM",
                    "data source issue",
                    f"Computed {opm:.4f} vs source {g(p,'opm_percentage'):.4f}; diff {diff:.4f} pp",
                )
            )
        ...
        fin_roces = []
        for fid in sectors.loc[
            sectors.broad_sector.eq("Financials"), "company_id"
        ].tolist():
            if (fid, year) in pl.index and (fid, year) in bs.index:
                fp = pl.loc[(fid, year)]
                fb = bs.loc[(fid, year)]
                fv = return_on_capital_employed(
                    fnum(fp.get("operating_profit")),
                    fnum(fb.get("equity_capital")),
                    fnum(fb.get("reserves")),
                    fnum(fb.get("borrowings")),
                )
                if fv is not None:
                    fin_roces.append(fv)
        roce_benchmark = (
            (float(np.median(fin_roces)) if fin_roces else None)
            if sector == "Financials"
            else None
        )
        roce_relative = (
            (roce - roce_benchmark)
            if roce is not None and roce_benchmark is not None
            else None
        )
        ...
        patrat = []
        for yy, rr in (
            pl.loc[cid].iterrows() if cid in pl.index.get_level_values(0) else []
        ):
            pat = fnum(rr.get("net_profit"))
            cfo_v = (
                fnum(cf.loc[(cid, yy)]["operating_activity"])
                if (cid, yy) in cf.index
                else None
            )
            if pat is not None and pat != 0 and cfo_v is not None:
                patrat.append(cfo_v / pat)
        ...
        pattern = capital_allocation_pattern(
            ...
            cfi,
            cff,
            (cfo / npf if cfo is not None and npf not in (None, 0) else None),
        )
        ...
        def growth(metric):
            ...
            for yy, rr in (
                source.loc[cid].iterrows()
                if cid in source.index.get_level_values(0)
                else []
            ):
                val = {
                    "revenue": rr.get("sales"),
                    "pat": rr.get("net_profit"),
                    "eps": rr.get("eps"),
                }[metric]
                yi = year_int(yy)
                val = fnum(val)
                if yi is not None and val is not None:
                    series.append((yi, val))
            out = {}
            ...
            for n in (3, 5, 10):
                cand = [
                    (y, v)
                    for y, v in series
                    if endyear is not None and y <= endyear - n
                ]
                if not cand:
        ...
        if (
            roce is not None
            and source_roce.get(cid) is not None
            and abs(roce - source_roce[cid]) > 5
        ):
            log_rows.append(
                (
                    cid,
                    ...
                    "ROCE",
                    "data source issue",
                    f"Computed {roce:.4f} vs company source {source_roce[cid]:.4f}; difference {roce-source_roce[cid]:.4f} pp",
                )
            )
        if (
            roe is not None
            and source_roe.get(cid) is not None
            and abs(roe - source_roe[cid]) > 5
        ):
            ...
            log_rows.append(
                (
                    ...
                    "ROE",
                    cat,
                    f"Computed {roe:.4f} vs company source {source_roe[cid]:.4f}; difference {roe-source_roe[cid]:.4f} pp",
                )
            )
        # Composite score: transparent 0-100 quality score, using available dimensions.
        parts = []
        if roe is not None:
            parts.append(min(max(roe / 20 * 25, 0), 25))
        if opm is not None:
            parts.append(min(max(opm / 20 * 20, 0), 20))
        if de is not None:
            parts.append(20 if de <= 1 else max(0, 20 - (de - 1) * 4))
        if icr is not None:
            parts.append(min(max(icr / 5 * 15, 0), 15))
        if score is not None:
            parts.append(min(max(score / 1.5 * 20, 0), 20))
    ...
    cols = [
        ...
        "roce_sector_benchmark_pct",
        "roce_vs_sector_benchmark_pct",
    ...
    for metric in ("revenue", "pat", "eps"):
        for n in (3, 5, 10):
            cols += [f"{metric}_cagr_{n}yr", f"{metric}_cagr_{n}yr_flag"]
    ...
    cfo2 = cf.reset_index()
    alloc2 = cfo2.merge(
        df[["company_id", "year", "capital_allocation_pattern"]],
        on=["company_id", "year"],
        how="left",
    )
    ...
    alloc2["cfo_sign"] = alloc2.operating_activity.map(sign)
    alloc2["cfi_sign"] = alloc2.investing_activity.map(sign)
    alloc2["cff_sign"] = alloc2.financing_activity.map(sign)
    alloc2[
        [
            "company_id",
            "year",
    ...
    ].rename(columns={"capital_allocation_pattern": "pattern_label"}).to_csv(
        OUT / "capital_allocation.csv", index=False
    )
    with open(LOG, "w", encoding="utf8") as fh:
        fh.write("# Sprint 2 ratio edge-case log\n")
        fh.write(
            "# Categories: data source issue, version difference, formula discrepancy\n"
        )
        for cid, year, metric, cat, msg in log_rows:
            fh.write(f"{cid} | {year} | {metric} | {cat} | {msg}\n")
        if not log_rows:
            fh.write("No anomalies detected.\n")
    conn.commit()
    # checks
...
if __name__ == "__main__":
    ...
    print(
        f"financial_ratios rows={count}; edge_case_entries={logs}; companies={df.company_id.nunique()}; columns={len(df.columns)}"
    )
